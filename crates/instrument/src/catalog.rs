...
impl DeviceCatalog {
    ...
    pub fn open_oscilloscope(&self, address: &str) -> Result<Oscilloscope> {
        self.device(address)?.open_oscilloscope()
    }

    pub fn open_switch(&self, address: &str) -> Result<Switch> {
        self.device(address)?.open_switch()
    }
